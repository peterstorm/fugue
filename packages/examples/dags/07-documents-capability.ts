...
// import `@fugue/document-source` (which augments `CapabilityRegistry`). At
// deploy time the host wires an adapter (@fugue/fs, @fugue/ms-graph, …); the
...
const fetchReport = createFetchNode({
  ...
  fetch: async (
    ...
    // @fugue/xlsx; this example just reports the byte count to stay dependency-light.
