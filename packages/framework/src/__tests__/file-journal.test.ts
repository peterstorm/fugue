  ...
  it("a fresh process appends successfully when the previous writer died holding events/append.lock", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        `// Publish readiness only AFTER acquiring the append lock. Publishing`,
        `// before withFileLock races the parent's SIGKILL against acquisition.`,
        `await withFileLock(dir + "/events/append.lock", () => {`,
