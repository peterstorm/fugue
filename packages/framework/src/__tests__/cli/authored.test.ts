...
describe("authored codegen survives the gauntlet", () => {
  ...
  it("stamps dag.ts with a tamper-evident @fugue-integrity hash of the STRUCTURE (loom's contract)", async () => {
    const root = join(tmpRoot, "integrity-stamp");
    // `linear` has fetch + transform nodes, so the projection excludes real
    // @fugue-body regions (an all-llm fixture would have none).
    const written = await writeAuthoredScaffold(mustParse(FIXTURES["linear"]!), { root, force: false }, [], []);
    if (!written.ok) throw new Error(written.problems.join("; "));

    const content = await readFile(join(written.dir, "dag.ts"), "utf-8");
    // Banner names the regenerate path so a human sees what to do.
    expect(content.startsWith("// @generated by `fugue new --from`")).toBe(true);
    expect(content).toContain("// @fugue-body-start");

    const m = /^\/\/ @fugue-integrity sha256:([0-9a-f]{64})$/m.exec(content);
    expect(m).not.toBeNull();
    const bodyOf = (c: string): string =>
      c.slice(c.indexOf("\n", c.indexOf(m![0]) + m![0].length) + 1);
    // Recompute exactly as loom's fugue-generated-integrity rule does: the hash
    // covers the STRUCTURAL PROJECTION (each @fugue-body region collapsed). A
    // byte drift here would silently break the cross-repo contract.
    expect(createHash("sha256").update(structuralProjection(bodyOf(content)), "utf-8").digest("hex")).toBe(m![1]);

    // The contradiction this design resolves: implementing a placeholder body
    // (what nextSteps + the README instruct) must NOT change the hash.
    const implemented = content.replace(
      /\/\/ @fugue-body-start[\s\S]*?\/\/ @fugue-body-end/,
      "// @fugue-body-start\n  fetch: async (i) => ok({ real: await work(i) }),\n  // @fugue-body-end",
    );
    expect(createHash("sha256").update(structuralProjection(bodyOf(implemented)), "utf-8").digest("hex")).toBe(m![1]);
