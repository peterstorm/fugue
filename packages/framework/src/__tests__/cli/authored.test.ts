...
describe("authored codegen survives the gauntlet", () => {
  ...
  it("stamps dag.ts with a tamper-evident @fugue-integrity hash of the body (loom's do-not-edit contract)", async () => {
    const root = join(tmpRoot, "integrity-stamp");
    const [, fixture] = Object.entries(FIXTURES)[0]!;
    const written = await writeAuthoredScaffold(mustParse(fixture), { root, force: false }, [], []);
    if (!written.ok) throw new Error(written.problems.join("; "));
    ...
    // covers the BODY (everything after the integrity line). A byte drift here
    // would silently break the cross-repo do-not-edit contract.
    const m = /^\/\/ @fugue-integrity sha256:([0-9a-f]{64})$/m.exec(content);
    expect(m).not.toBeNull();
    const markerEnd = content.indexOf(m![0]) + m![0].length;
    const body = content.slice(content.indexOf("\n", markerEnd) + 1);
    expect(createHash("sha256").update(body, "utf-8").digest("hex")).toBe(m![1]);
  });
