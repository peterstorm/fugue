    ...
    const witnessEvents: WitnessCapturedEvent[] = [
      {
        ...
        dagId: D("d"),
        nodeId: N("reader"),
        witness: witness("version", RN("postgres:orders"), "1"),
        capturedAtMs: 100,
      ...
      },
      {
        ...
        dagId: D("d"),
        nodeId: N("reader2"),
        witness: witness("version", RN("postgres:orders"), "2"),
        capturedAtMs: 300,
...
describe("Full pipeline: reads → freshness violation → human intervention", () => {
  test("human-intervention event carries stale witness in priorWitnesses after violation", async () => {
    ...
    expect(intervention!.context.priorWitnesses).toContainEqual(witness("version", RN("postgres:orders"), "42"));
  });
});
