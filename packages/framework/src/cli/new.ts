...
//                         truth the generated `dag.ts` is stamped against)
//
...
import { buildAuthoredScaffold, structuralProjection } from "./authored-codegen.js";
 ...
 * instruct. loom ships a `fugue-generated-integrity` lint rule that recomputes
 * this hash over the SAME projection and blocks on a mismatch at the wave gate —
 * the coupling is the two `@fugue-body` marker strings + the collapse rule,
 * nothing imported. The projected structure is the sole source of truth for the
 * hash, so regeneration from the same AuthoredDag is a byte-for-byte fixed point
 * (identical structure → identical hash), independent of the bodies.
 */
const stampGenerated = (body: string): string => {
