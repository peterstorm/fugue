 ...
 * `@fugue-integrity sha256:<hex>` hash covers the BODY (everything after the
 * banner), so a hand-edit during downstream orchestration (e.g. a loom wave)
 * changes the body but not the stamp. loom ships a `fugue-generated-integrity`
 * lint rule that recomputes this hash and blocks on a mismatch — the coupling
 * is this one marker-string convention, nothing imported. The body is the sole
 * source of truth for the hash, so regeneration from the same AuthoredDag is a
 * byte-for-byte fixed point (identical body → identical hash → identical file).
 */
