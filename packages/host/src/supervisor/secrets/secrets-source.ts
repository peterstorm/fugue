 ...
 * actual tenant secrets (AD-6, multi-tenant spec FR-031).
 *
 * ── The structural security property (multi-tenant spec FR-005, FR-006, SC-002) ─────────────────
 *
 ...
 *     metadata (multi-tenant spec FR-005). It may hold a `SecretsRef` (it rides in registry
 ...
 *     this port*, not even transiently (multi-tenant spec FR-006). This file's type is the ONLY
 ...
 *     ref INSIDE the worker process (multi-tenant spec FR-031). Because construction lives only
 *     there, the secret value exists only in worker memory — the
 *     supervisor-secrets-absence guarantee (multi-tenant spec SC-002) holds by construction, not
 *     by convention.
 *
 ...
 *
 * @satisfies multi-tenant spec FR-005 — the supervisor does not import/construct a `SecretsSource` (convention today, verified e2e in T11); it holds only the inert `SecretsRef`.
 * @satisfies multi-tenant spec FR-006 — tenant env-file secrets resolve only inside the owning worker; no such value transits the supervisor via this port, even transiently.
 * @satisfies multi-tenant spec FR-031 — the per-tenant secrets source is referenced by registry metadata and resolved only within the owning worker.
 * @satisfies multi-tenant spec SC-002 — resolution requires a concrete adapter (filesystem/Vault authority) the supervisor structurally lacks.
 */
 ...
 *
 * Fail-closed (multi-tenant spec FR-006): a missing, unreadable, or malformed source yields a
