...
const makeDag = (id: string, overrides?: Partial<RegisteredDag>): RegisteredDag => ({
  ...
  config: { route: `/run/${id}`, timeout: 30_000, maxConcurrency: 10 },
  ...
  describe("property tests", () => {
    ...
    const arbRegisteredDag = arbDagId.chain((id) =>
      fc.record({
        ...
        config: fc.constant({ route: `/run/${id}`, timeout: 30_000, maxConcurrency: 10 }),
        meta: fc.constant({ description: "", version: "0.0.0" }),
        loadedAt: fc.nat(),
        sha: arbSha,
        status: fc.oneof(
