...
describe("error-handler middleware", () => {
  ...
  describe("HostError runtime recognition", () => {
    ...
    const validVariants: readonly HostError[] = [
      ...
      { kind: "circuit-open", dagId: "dag" as never, retryAfterSeconds: 30 as never },
      { kind: "global-concurrency-exceeded" },
    ...
    it.each([
      ...
      ["circuit-open.dagId", { kind: "circuit-open", dagId: "bad id", retryAfterSeconds: 30 }],
      ["circuit-open.retryAfterSeconds negative", { kind: "circuit-open", dagId: "dag", retryAfterSeconds: -1 }],
      ["circuit-open.retryAfterSeconds fractional", { kind: "circuit-open", dagId: "dag", retryAfterSeconds: 1.5 }],
      ["circuit-open.retryAfterSeconds NaN", { kind: "circuit-open", dagId: "dag", retryAfterSeconds: Number.NaN }],
      ["circuit-open.retryAfterSeconds non-number", { kind: "circuit-open", dagId: "dag", retryAfterSeconds: "30" }],
