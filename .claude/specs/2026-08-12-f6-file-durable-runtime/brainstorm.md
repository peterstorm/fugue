...
7. **Suite parity.** `createFileCheckpointer` must pass the shared `checkpointerSuite`: ADR-0017 version-mismatch rejection (FRAMEWORK_VERSION), `expectedDagFingerprint` opt-in, 24h TTL expiry, per-entry corrupt drop with `corruptNodeAddresses` (per-node files make per-entry drop natural; a single aggregate node file would not).
