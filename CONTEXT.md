...
| **Run Spend Authority** | The one mutable accounting cell for an execution slice. Main `llm`, `judgeLlm`, every boot-scoped `clientKind: "llm"` capability, and every broker-delivered tagged LLM share its meter, reservations, ledger, logs, and Budget Capability. The metered boundary parses one frozen own-data request snapshot for both pricing and provider egress. Broker values form a closed `llm | non-llm` binding ADT; node requirements and broker claims are snapshotted before the awaited mint, and every delivered key must be present in both snapshots. Every scoped LLM carries a required declarative alias map, so stateful requests, broker mutation, over-delivery, untagged clients, and missing augmented aliases fail before provider/context authority can drift. Each client carries an authority-owned pricing model; fixed bindings reject a conflicting request before egress and settlement prices the effective model. Azure routing deployment names remain separate from these underlying pricing-model SKUs. |
...
| **Spend Ledger** | Port holding a run's DURABLE spend, keyed by `runId`. The Run Spend Authority is authoritative WITHIN an execution slice; the ledger carries spend BETWEEN slices. Additive appends are never retried or automatically replayed after ambiguous acknowledgement; production ioredis disables `autoResendUnfulfilledCommands` (ADR-0083). |
