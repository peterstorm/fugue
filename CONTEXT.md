...
| **Spend Record** | The Redis ledger's ONE-hash encoding: `micros`/`tokens`/`calls` numeric fields plus reserved `$usage:unknown` and `$unpriced:<encoded-model>` marker fields. One optimistic `WATCH`/`MULTI` transaction reads and saturates numeric axes at `Number.MAX_SAFE_INTEGER`, then queues markers first, numeric replacements cost-first, and one hash expiry; only proven `EXEC null` conflicts retry. One strict `HGETALL` rejects unknown/malformed fields and preserves canonical model sorting. A checkpoint transaction writes the checkpoint with its DAG TTL and refreshes spend to the longer of checkpoint and authoritative resumable-run retention, so HITL state cannot outlive accounting. |
