...
| **CapabilityHandle\<K\>** | Runtime lifecycle wrapper: `{ name, client, connect?, close?, healthCheck? }`. A handle whose registered client extends `LlmClient` also requires `clientKind: "llm"`; an augmented LLM subtype additionally requires a declarative `runScopedOperations` alias map. The host interprets that data into a frozen run-scoped facade whose provider operations all delegate through the metered standard surface. |
...
| **Run Spend Authority** | The one mutable accounting cell for an execution slice. Main `llm`, `judgeLlm`, and every `clientKind: "llm"` capability-bag client share its meter, reservations, ledger, logs, and Budget Capability; the host binds augmented aliases from `runScopedOperations` directly to its narrow metered surface. |
...
| **Spend Record** | The Redis ledger's ONE-hash encoding: existing `micros`/`tokens`/`calls` numeric fields plus reserved `$unpriced:<encoded-model>` marker fields. One `MULTI`/`EXEC` queues markers first, numeric fields cost-first, then one hash expiry; one strict `HGETALL` rejects unknown/malformed fields and preserves canonical model sorting/dedup. No second key/read can race expiry. |
