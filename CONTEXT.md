...
| **Run Spend Authority** | The one mutable accounting cell for an execution slice. Main `llm`, `judgeLlm`, every boot-scoped `clientKind: "llm"` capability, and every explicitly marked broker-delivered LLM share its meter, reservations, ledger, logs, and Budget Capability. Each client carries an authority-owned pricing model; fixed bindings reject a conflicting request before egress and settlement prices the effective model. The host binds augmented aliases from `runScopedOperations` directly to its narrow metered surface. |
...
| **Spend Record** | The Redis ledger's ONE-hash encoding: `micros`/`tokens`/`calls` numeric fields plus reserved `$usage:unknown` and `$unpriced:<encoded-model>` marker fields. One `MULTI`/`EXEC` queues markers first, numeric fields cost-first, then one hash expiry; one strict `HGETALL` rejects unknown/malformed fields and preserves canonical model sorting. Checkpoint writes with retention atomically write the checkpoint and refresh the spend hash to the same deadline, so resumable state cannot outlive accounting. |
