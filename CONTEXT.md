...
| **Run Spend Authority** | The one mutable accounting cell for an execution slice. Main `llm`, `judgeLlm`, and every `clientKind: "llm"` capability-bag client share its meter, reservations, ledger, logs, and Budget Capability; augmented facades receive its narrow metered surface through `composeRunClient`. |
