...
| **AuthoredDag** | Closed authoring-direction DAG value issued only by `parseAuthoredDag` / `parseAuthoredDagJson`. Successful parsing owns and recursively freezes the complete data graph before branding it, so the validation proof cannot be invalidated by later type-correct mutation. Deterministic scaffolds remain importable and lintable, but untouched fetch/source/transform body regions return a typed unimplemented error rather than fabricated successful data. Their `$fugue` namespace is used by machine-owned factories and exposes `ok`/`err`/`frameworkError` inside editable bodies, so completing every body needs no integrity-hashed import edit and creates no dead failure import. Every authored DAG name, child DAG id, node id, and structural reference shares the runtime identifier maximum, so the parser-issued proof cannot later fail module construction on identifier length. |
