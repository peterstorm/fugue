...
| **Spend Record** | The Redis ledger's ONE-hash encoding: `micros`/`tokens`/`calls` numeric fields plus reserved `$usage:unknown` and `$unpriced:<encoded-model>` marker fields. One `MULTI`/`EXEC` queues markers first, numeric fields cost-first, then one hash expiry; one strict `HGETALL` rejects unknown/malformed fields and preserves canonical model sorting. A checkpoint transaction writes the checkpoint with its DAG TTL and refreshes spend to the longer of checkpoint and authoritative resumable-run retention, so HITL state cannot outlive accounting. |
