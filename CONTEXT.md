...
| **MicroUsd** | Non-negative safe-integer 1e-6 USD, branded. What ceilings compare against; raw USD floats stay display-only. Addition/projection saturate at `Number.MAX_SAFE_INTEGER`, which fails closed against every valid monetary ceiling while avoiding settlement-order float drift. |
...
| **Run Spend Authority** | The one mutable accounting cell for an execution slice. Main `llm`, `judgeLlm`, every boot-scoped `clientKind: "llm"` capability, and every broker-delivered tagged LLM share its meter, reservations, ledger, logs, and Budget Capability. Broker values form a closed `llm | non-llm` binding ADT; every scoped LLM carries a required declarative alias map, so untagged clients and missing augmented aliases fail before context merge. Each client carries an authority-owned pricing model; fixed bindings reject a conflicting request before egress and settlement prices the effective model. |
