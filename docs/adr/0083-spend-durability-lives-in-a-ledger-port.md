...
**The encoding preserves `addSpend`'s commutative axes within Redis's safe-integer storage domain.** `domain/spend-record.ts` flattens a `Spend` into ONE Redis hash: three established numeric fields plus reserved `$usage:unknown` and `$unpriced:<canonical-encoded-model>` fields whose value is exactly `1`. Usage uncertainty and the `priced`/`unpriced` discriminant are **derived from marker-field presence** rather than stored as mutable tags. Redis queues all marker `HSET`s first, nonzero `HINCRBY` fields cost-first (`micros`, `tokens`, `calls`), then ONE `EXPIRE` on that hash when retention is configured. Unlike in-process `addSpend`, Redis `HINCRBY` does not saturate at `Number.MAX_SAFE_INTEGER`: a cumulative axis may cross that bound. Hydration then strictly rejects the unsafe integer as a typed read failure, so a budgeted slice fails closed rather than hydrating a wrapped or imprecise total. Concurrent transactions remain correct below that bound because sums and marker-field union commute. No lock, `WATCH`, retry loop, Lua, second key, or second read is used.
