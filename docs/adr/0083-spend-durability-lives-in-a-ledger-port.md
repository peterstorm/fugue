...
- **Each individual partial append remains directional.** Its commands run cost-first and fail fast, so the dollar axis is recorded before tokens, calls, and unpriced model names. An interrupted append can under-report one or more later axes for that call; it never over-reports or corrupts the stored algebra.
