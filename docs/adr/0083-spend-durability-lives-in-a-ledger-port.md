...
- The spend hash lives under the same `fugue:<tenant>:` prefix as every other key, so the per-tenant ACL scopes it unchanged. Its one TTL follows the checkpoint TTL and refreshes inside the append transaction. A later checkpoint write atomically writes that checkpoint and refreshes spend retention to the same deadline; there is no crash window where newly resumable state outlives its accounting. **No configured checkpoint TTL means no spend expiry**.
