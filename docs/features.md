...
- When backed by a durable `Checkpointer`, every successfully acknowledged index is retained under map/index/execution-epoch addressing, so process replacement reruns only missing work. Child effects completed before a lost fan-save acknowledgement may repeat.
